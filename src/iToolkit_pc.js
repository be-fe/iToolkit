...
riot.tag('paginate', '<div class="paginate" if="{ currentPage != 1 }"> <li>首页</li> <li>上一页</li> </div> <ul class="paginate" if="{ pageCount > 1 }"> <li each="{ pages }" onclick="{ parent.changePage }" class="{ active: parent.currentPage == page }">{ page }</li> </ul> <div class="paginate" if="{ currentPage != pageCount }"> <li>下一页</li> <li>末页</li> </div>', '.paginate { display: inline-block; padding: 0; } .paginate li { cursor: pointer; display: inline-block; position: relative; padding: 6px 12px; margin-left: -1px; line-height: 1.42857143; color: #337ab7; text-decoration: none; background-color: #fff; border: 1px solid #ddd; } .paginate li.active { color: #23527c; background-color: #eee; border-color: #ddd; }', function(opts) {
...
riot.tag('tab', '<ul> <li each="{ data }" onclick="{ parent.toggle }">{ title }</li> </ul> <div class="tab-content"> { content } </div>', function(opts) {
